...
function selectMode(&$modeName,$controll = '') {
    ...
    if (in_array(SCR, array('index', 'mode'))) {
        ...
        if (defined('M_P') && !defined('HTML_CHANNEL') && !empty($db_modedomain[$db_mode]) && $db_modedomain[$db_mode] !== $pwServer['HTTP_HOST']) {
            if (strpos($GLOBALS['pwserver']['SCRIPT_NAME'],'mode.php') !== false)
...
function GetLang($lang, $EXT = 'php') {
    ...
    if ($GLOBALS['db_debug']) {
        ...
        $langfile = file_exists(R_P . "lang/wind/lang_$lang.php") ? R_P . "lang/wind/lang_$lang.php" : '';
        if (!file_exists($path)) {
            !$langfile && exit("Can not find lang_$lang.$EXT file");
            creat_langfile($langfile, R_P . "template/wind/lang_$lang.php");
...
function PrintEot($template, $EXT = 'htm') {
    ...
    if ($GLOBALS['db_debug']) {
        ...
        include_once (R_P . 'lang_change.php');
        if ($db_tplpath && $db_tplstyle) {
            $tmplangfile1 = R_P . "lang/wind/mode/$db_mode/themes/$db_tplstyle/$template.$EXT";
            $tmplangfile2 = M_P . "themes/$db_tplstyle/$template.$EXT";
        } elseif (defined('M_P')) {
            $tmplangfile1 = R_P . "lang/wind/mode/$db_mode/$template.htm";
            $tmplangfile2 = M_P . "template/$template.htm";
        }

        if (defined('A_P') && $appdir && substr($template,0,2) == 'm_') {
            $tmplangfile1 = R_P . "lang/wind/apps/$appdir/$template.htm";
            $tmplangfile2 = A_P . "$appdir/template/$template.htm";
        }

        if (defined('A_P') && file_exists($tmplangfile1)) {
            creat_tplfile($tmplangfile1, $tmplangfile2);
        } elseif (defined('M_P') && file_exists($tmplangfile1)) {
            creat_tplfile($tmplangfile1, $tmplangfile2);
        } elseif (file_exists(R_P . "lang/wind/$tplpath/$template.htm")) {
            creat_tplfile(R_P . "lang/wind/$tplpath/$template.htm", R_P . "template/$tplpath/$template.htm");
        } elseif (file_exists(R_P . "lang/wind/wind/$template.htm")) {
            creat_tplfile(R_P . "lang/wind/wind/$template.htm", R_P . "template/wind/$template.htm");
        }
    }
